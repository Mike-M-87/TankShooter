extends Node2D

onready var TankSprite = $Sprite

var tank = {
	"rectx":678,
	"recty":105,
	"rectw":245,
	"recth":282,
	"button1text":"BUY 500/=",
	"button2text":"BUY 2000/=",
	"bar_rectx":1040.047,
	"bar_recty":64.534,
	"bar_rectw":125,
	"bar_recth":270,
	"probarreltext":"BUY 300/=",
	"ultbarreltext":"BUY 1000/=",
	"protank_purchased":false,
	"ultimatetank_purchased":false,
	"probarrel_purchased":false,
	"ultbarrel_purchased":false,
	"tank_damage":20,
	"tank_speed":400,
	"tank_health":2,
	"barrel_rate":0.4,
	"kills":0
	}

func _ready():
	TankSprite.texture = preload("res://Sprites/Tanks.png")
	TankSprite.region_enabled = true
	if load_data():
		TankSprite.region_rect = Rect2(tank.rectx,tank.recty,tank.rectw,tank.recth)

func _process(delta):
	pass

func load_data():
	var file = File.new()
	if !file.file_exists("user://tank"):
		return false
	file.open("user://tank",File.READ_WRITE)
	tank = file.get_var()
	file.close()
	return true
